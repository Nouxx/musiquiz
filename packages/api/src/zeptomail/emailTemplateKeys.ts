type EmailTemplateKey = {
  clientTemplateKey: string;
  internalMailTemplateKey: string;
};

export const emailTemplateKeys: Record<string, EmailTemplateKey> = {
  clientContact: {
    clientTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.60f89490-a6d3-11f1-a4b9-dad70ff08860.1a062565c59",
    internalMailTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.155fb4a0-a6d3-11f1-a4b9-dad70ff08860.1a062546cea",
  },
  teamBuilding: {
    clientTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.58ef8070-a6d2-11f1-a4b9-dad70ff08860.1a0624f99f7",
    internalMailTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.155fb4a0-a6d3-11f1-a4b9-dad70ff08860.1a062546cea",
  },
  musiTeens: {
    clientTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.9d81f670-aac7-11f1-819f-66e0c45c7bae.1a07c402757",
    internalMailTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.9203c350-aac7-11f1-819f-66e0c45c7bae.1a07c3fdc05",
  },
  newVenue: {
    clientTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.60f89490-a6d3-11f1-a4b9-dad70ff08860.1a062565c59",
    internalMailTemplateKey:
      "13ef.34e5f5b7aef4f343.k1.675c92d0-aac7-11f1-819f-66e0c45c7bae.1a07c3ec47d",
  },
};
