...
- **The preview build deliberately does the opposite.** `image.domains` is empty when `isPreview`, so `getURL` returns raw CDN URLs and the Cloudflare worker does not proxy image bytes it would not transform anyway (the adapter runs `imageService: "passthrough"`). Sanity is up by definition when previewing drafts. `RemoteImage` is byte-identical across both builds; only config forks.
