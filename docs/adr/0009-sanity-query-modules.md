...
And a `Sanity*` type **stops at the adapter that consumes it**. `@repo/services` imports them in exactly two places: the `getXData.ts` that calls a fetch function, and the `utils/toX.ts` that maps one shape to the other. `pageComponent.types.ts` and `types.ts` — the types `apps/web` reads — import nothing from `@repo/api`, and a type in them is written out by hand even when a Query Module already holds the same shape. Deriving one, `icon: SanityDetailGroup["cards"][number]["icon"]`, was tried and reverted: the CMS is one source behind these types and the point of the layer is that it can be swapped, so a field's type is the contract's decision, not a projection's. The duplication is small, and the adapter is where the two are forced to agree — the mapping stops compiling the moment they diverge, which is the check that derivation was reaching for.
...
`shared/` is **open, on one test**: a fragment belongs there when it describes a named thing whose shape is fixed by the domain rather than by the page asking for it — a CMS Image, a Page Component, a call to action. That is a judgement about the fragment, not a repeat count, and it needs no ADR to add the next one. Everything failing that test stays duplicated across Query Modules on purpose: schemas are `z.strictObject`, so a shared projection is a coupling, not a dedup. Adding a field to a shared fragment forces the key into every schema that uses it or `parse` throws at build. The footer and the venue footer both project the site's social URLs today; sharing that block would mean a Bluesky link on the venue footer editing the site footer's schema. Each Query Module keeps full freedom to ask for exactly the fields its page needs, and a surgical change stays surgical.
...
- **A field the projection never selects arrives as `undefined`, and `.nullable()` refuses it.** The two are not interchangeable: GROQ's `select()` with no matching case resolves to `null`, which is why `optionalCtaProjection` pairs with `sanityCtaSchema.nullable()` and works — but a key that is simply absent from the projection object is `undefined`, and `z.…nullable()` rejects that with `expected object, received undefined`, pointing at the schema rather than at the projection that forgot it. So the rule is **`.nullable()` for a field the projection asks for and GROQ may resolve to nothing, `.optional()` for nothing at all** — if a schema needs `.optional()`, the honest fix is almost always to delete the field, because the query is not asking for it. The corollary is the useful one when the error appears: read it as "the projection and the schema disagree about this key", and check the projection first. `z.strictObject` catches the mirror-image mistake, a key projected but not declared.
...
- **Duplication between `footer.ts` and `venueFooter.ts` is expected and must not be refactored away.** Same for the game-link projection shared by `venueFooter.ts` and `header.ts`. Neither names a thing the domain fixes the shape of, which is the test for promoting a fragment to `shared/` — not a repeat count.
