...
Values reach GROQ as **parameters**, never string interpolation. Queries used to splice their arguments into the query text (`slug.current == "${venueSlug}"`), which made every language and every venue a distinct query string and made the text opaque to `sanity typegen` should that decision ever be revisited. Distinct query strings defeat any cache keyed on them, which is moot today — `client.ts` sets `useCdn: false` on purpose, the site being SSG or SSR-for-preview — and stops being moot the moment that flips. It was also GROQ injection in shape: not exploitable today — slugs come from `getVenueSlugs()` against our own dataset at build time, and GROQ is read-only — but one route change away from being fed a URL segment, and that route change would not look dangerous. With `$lang` and `$venueSlug` the query text is constant, so queries are `const` strings rather than functions returning template literals, and the shared projections are constants too.

The fetch function is **written by hand**, not generated. `fetchSanityData` stays a plain helper taking `query`, `schema`, `parameters` and `config`, and each module wraps it in a four-line `async function` that names its own arguments:

```ts
export async function fetchFooter({ config, lang }: { config: SanityConfig; lang: Lang }) {
  return fetchSanityData({ query: footerQuery, schema: sanityFooterSchema, parameters: { lang }, config });
}
```

A factory was tried first — `defineSanityQuery<{ lang: Lang }>()({ query, schema })` — and removed. It saved those four lines and charged for them: currying, because TypeScript cannot infer the schema while the parameter type is supplied explicitly; a `Record<never, never>` default for the module that takes no parameters; a rest-spread over a generic that did not typecheck against the client's conditional params overload; and an exception to the repo's own rule that functions are declared with `function`. The wrapper costs four lines per module and is the thing everyone already knows how to read.

It also puts `parameters: { lang }` on screen beside the query text that reads `$lang`. Nothing type-checks that pairing — a factory could not have checked it either — so the next best thing is that both are in one file, a few lines apart.
