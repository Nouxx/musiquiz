...
**Amended by [ADR 0006](./0006-design-tokens-from-figma-sheets.md)**: Type Tokens are single-tier and named for the Figma sheet's steps (`--title-h1`), not intent (`--text-title`). Colour keeps the two tiers described here.

## Consequences

- A re-skin or dark mode is a token-layer edit, no component changes.
- Typography is delivered via the Astro Fonts API into `--font-*`, exposed as composite Type Tokens, and consumed through a `Text` UI Component (variant → token) rather than raw font rules.
