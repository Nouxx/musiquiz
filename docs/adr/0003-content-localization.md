...
- An **optional** localized field cannot fail loudly, because `null` is its legitimate empty state. `pageCoverBadge` and `price.note` both project `.nullable()`, so a note authored in French and never translated is indistinguishable from a note nobody wrote, and the English page drops it in silence. That is the price of optionality under field-level localization, not an oversight: the alternative is a required field with an empty string, which fails a `.min(1)` check the moment an editor leaves it blank on purpose. Keep the list of `.nullable()` localized fields short, and reach for it only when the field is genuinely optional for the _content_, never to paper over a missing translation.
...
- `fieldTypes: ["string", "text"]` are the registered types, so `internationalizedArrayString` and `internationalizedArrayText` are the localized fields available — the second added for the prices footnote, whose body is a paragraph a one-line input cannot hold. Reach for `Text` when the copy is prose and `String` when it is a line. Localized Portable Text still needs the array extended again.
