...
> Revised: the legal pages carry no Page Component array. The design fixes their content to one block of rich text, so an array would offer choices the design forbids; each holds a required `body` rich text field instead.
